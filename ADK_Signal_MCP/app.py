"""
Signal MCP Server — ASGI entry point.
...
Run:
    .venv/bin/uvicorn app:app --host 0.0.0.0 --port 8020 --workers 2
"""
...
async def _build_cache() -> None:
    ...
    logger.info(
        "Signal MCP ready — tools=%d resources=%d prompts=%d",
...
async def root(request: Request) -> Response:
    return JSONResponse({
        ...
        "version": _VERSION,
        "description": (
            "MCP server for Walmart International Signal — app inventory, "
            "metadata filters, and OE reports"
        ),
        "mcp_endpoint": "/mcp",
...
def main() -> None:
    uvicorn.run(
        ...
        port=int(os.environ.get("APP_PORT", "8020")),
